...
The report consumes the CLI's snapshot archive, but the two have separate toolchains: Go for the CLI, Python for the report generator, Node/Sass for the USWDS theme build. See `README.md` for full user-facing docs on both; this file is the cross-file "how it fits together" that isn't obvious from any single file.
...
There is no Go test suite yet (`find . -name '*_test.go'` is empty).

### Report pipeline (regenerates `docs/index.html`)

```bash
./uswds-usage report                    # writes snapshots/<today>/
python3 scripts/refresh_gsa_history.py  # incremental; appends new GSA commits only
npm install && npm run build            # compiles theme/ -> docs/assets/uswds/
python3 scripts/build_report.py         # writes docs/index.html
```
...
`main.go` does its own arg-based dispatch — no cobra/cli framework. Bare `uswds-usage` or `uswds-usage -flag` is a backward-compatible alias for `report` (this predates the other subcommands and must keep working unmodified). Each subcommand (`report` in `main.go`, `backfill.go`, `trend.go`, `serve.go`) owns its own `flag.NewFlagSet`.
...
- `gsa-uswds-report-history.csv` — GSA's own daily cohort-level adoption report, extracted from that repo's commit history via `scripts/refresh_gsa_history.py`. Has a ~10x methodology-break jump on 2026-03-25 (a GSA bug fix) and one broken scan day (2026-06-26).
...
- `scripts/__pycache__/` is currently **not** gitignored and has been committed at least once — if you see a stale `.pyc` causing confusing behavior from `build_report.py`, that's why.
